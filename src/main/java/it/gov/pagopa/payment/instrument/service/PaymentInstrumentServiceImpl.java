...
public class PaymentInstrumentServiceImpl implements PaymentInstrumentService {
  ...
  public void enrollInstrument(String initiativeId, String userId, String idWallet,
    ...
    for (PaymentInstrument pi : instrumentList) {
      if (!pi.getUserId().equals(userId)) {
        ...
        utilities.logEnrollInstrumentKO(PaymentInstrumentConstants.ERROR_PAYMENT_INSTRUMENT_ALREADY_ACTIVE, idWallet, pi.getHpan(), channel);
      ...
      if (pi.getInitiativeId().equals(initiativeId) && !pi.getStatus().equals(PaymentInstrumentConstants.STATUS_ENROLLMENT_FAILED_KO_RE)) {
        ...
        performanceLog(startTime, ENROLL_INSTRUMENT);
        utilities.logEnrollInstrumentKO("already active or in pending", idWallet, pi.getHpan(), channel);
        return;
      }
    }
    ...
    try {
      ...
      utilities.logEnrollInstrumentKO(e.getMessage(), newInstrument.getIdWallet(), newInstrument.getHpan(), channel);
    ...
    utilities.logEnrollInstrumentComplete(newInstrument.getIdWallet(), newInstrument.getHpan(), newInstrument.getChannel());
    performanceLog(startTime, ENROLL_INSTRUMENT);
  }
  private void enrollInstrumentFailed(PaymentInstrument instrument,RTDHpanListDTO hpanListDTO, String channel){
    try {
      ...
      utilities.logEnrollInstrumentKO(e.getMessage(), instrument.getIdWallet(), instrument.getHpan(), channel);
      paymentInstrumentRepository.delete(instrument);
      throw new PaymentInstrumentException(HttpStatus.BAD_REQUEST.value(), e.getMessage());
    }
    utilities.logEnrollInstrumentComplete(instrument.getIdWallet(), instrument.getHpan(), channel);
  }
  ...
  private void checkPendingTimeLimit() {
    ...
    for(PaymentInstrument instrument: instruments){
      if(timeStampNow.isAfter(instrument.getUpdateDate().plusHours(4))){
        ...
        instrument.setUpdateDate(LocalDateTime.now());
        paymentInstrumentRepository.save(instrument);
        utilities.logEnrollInstrumentKO("Pending Time Limit expired", instrument.getIdWallet(), instrument.getHpan(), instrument.getChannel());
      }
  ...
  private void checkAndDelete(PaymentInstrument instrument,
    ...
    paymentInstrumentRepository.save(instrument);
    utilities.logDeactivationComplete(instrument.getIdWallet(), infoList.getHpan(), instrument.getChannel(), deactivationDate);
  ...
  private void processAckDeactivate(RuleEngineAckDTO ruleEngineAckDTO) {
    ...
    if (!ruleEngineAckDTO.getHpanList().isEmpty()) {
      ...
      utilities.logDeactivationComplete(instrument.getIdWallet(), instrument.getHpan(), instrument.getChannel(), LocalDateTime.now());
    ...
    if (!ruleEngineAckDTO.getRejectedHpanList().isEmpty()) {
      ...
      instrument.setStatus(PaymentInstrumentConstants.STATUS_ACTIVE);
      paymentInstrumentRepository.save(instrument);
      utilities.logDeactivationKO(instrument.getIdWallet(), instrument.getHpan(), instrument.getChannel(), LocalDateTime.now());
    }
  ...
  private void processAckEnroll(RuleEngineAckDTO ruleEngineAckDTO) {
    ...
    if(status.equals(PaymentInstrumentConstants.STATUS_ENROLLMENT_FAILED_KO_RE)){
      ...
      utilities.logAckEnrollKO(instrument.getIdWallet(), instrument.getHpan(), instrument.getChannel(), ruleEngineAckDTO.getTimestamp());
    ...
    if(status.equals(PaymentInstrumentConstants.STATUS_ACTIVE)) {
      ...
      utilities.logAckEnrollComplete(instrument.getIdWallet(), instrument.getHpan(), instrument.getChannel(), instrument.getActivationDate());
    }
  ...
  public void rollbackInstruments(List<PaymentInstrument> paymentInstrumentList) {
    ...
    for (PaymentInstrument instrument : paymentInstrumentList) {
      ...
      instrument.setUpdateDate(LocalDateTime.now());
      utilities.logDeactivationKO(instrument.getIdWallet(), instrument.getHpan(), instrument.getChannel(), instrument.getUpdateDate());
    }
    paymentInstrumentRepository.saveAll(paymentInstrumentList);
